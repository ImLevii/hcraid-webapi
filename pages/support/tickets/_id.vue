	...
	<div class="uk-container uk-margin">
		<template v-if="ticket != null">
			<ul class="uk-breadcrumb">
				...
				<li>
					<nuxt-link :to="{ name: 'support-tickets-id', params: { id: ticket.id } }">{{ ticket.title }}</nuxt-link>
				</li>
			</ul>
						...
						<div class="ticket">
							...
							<div class="ticket-body">
								<div class="ticket-details">
									<SupportTicketLink :ticket="ticket" :charLimit="70" class="page-title ticket-title"/>
...
<script>
    ...
    export default {
        ...
        async asyncData({$axios, route, error}) {
            ...
            return await $axios.get('/user/ticket/' + id).then(response => {
		...
		methods: {
            submitReply() {
	            ...
	            this.$axios.post('/user/tickets/reply', {
	                'ticket_id': this.ticket.id,
					'details': this.replyInput,
				}).then(response => {
				    if (response.status === 201) {
				        this.replyInput = "";
				        this.ticket['ticket_status'] = 'USER_REPLY';
